...
const spring = function(
  ...
  const start = function(
    ...
    console.warn("wechat mini is not support spring animation now!")
    // callback = _combineCallbacks(callback, configuration);
    // const singleValue: any = animatedValue;
    // const singleConfig: any = configuration;
    // // 注入config
    // singleValue.config = config;
...
const timing = function(
  ...
  const start = function(
    ...
    if (configuration.toValue instanceof AnimatedNode) {
      ...
      // 重置初始化时强行挂载的 config，否则会导致下一次动画残留之前的效果
      const resetConfig = ()=>{
        singleValue.config = undefined;
      }
      // console.log("onUpdate------",singleValue)
      singleValue.animate(new TimingAnimation(singleConfig), callback,resetConfig);
    }
  };
...
const decay = function(
  ...
  const start = function(
    ...
    console.warn("wechat mini is not support decay animation now!")
    // callback = _combineCallbacks(callback, configuration);
    // const singleValue: any = animatedValue;
    // const singleConfig: any = configuration;
    // singleValue.stopTracking();
    ...
    start: function(callback?: ?EndCallback) {
      const restart = function(result: EndResult = {finished: true}): void {
        if (
          ...
          // 延迟处理，否者会导致restart 和 start 两次setState合并，循环失败
          setTimeout(()=>animation.start(restart),20)
        }
      };
