 ...
 */
function _flush(rootNode: AnimatedValue,value): void {
  ...
  /* $FlowFixMe */
  animatedStyles.forEach(animatedStyle => animatedStyle.update(value));
}
...
class AnimatedValue extends AnimatedWithChildren {
  ...
  setValue(value: number): void {
    ...
    this._updateValue(
      value,
      true /* don't perform a flush for natively driven values */,
    );
  }
   ...
   */
  animate(animation: Animation, callback: ?EndCallback,resetConfig): void {
    let handle = null;
    ...
    animation.start(
      this._value,
        ...
        this._updateValue(value, true /* flush */,true);
      },
      result => {
        this._animation = null;
        if (handle !== null) {
  ...
  _updateValue(value: number, flush: boolean,isStart): void {
    this._value = value;
    if (flush) {
      _flush(this,isStart);
    }
